REM Default Printer Check LOGOFF
REM Version	0.9.0
...
REM This scripts logs every printer that is mapped when the user logs off from it's virtual desktop
...
:NoXDClientName
ECHO ERROR: %date% %time% on %computername% SIN VARIABLE DE ENTORNO at LogOFF>> %1
ECHO ---------------------------------------------------------- >> %1
ECHO. >> %1
GOTO END

:SinParametro
ECHO Ussage: DefaultPrnCheck_LogOFF.cmd path_to_log_file.log
GOTO END

:END
REM XDClientName environmental variable is erased from Registry to ensure that it will be correctly loaded at next logon.
REM Otherwise, incorrect data may be logged:
CALL REG delete HKCU\Environment /V XDClientName /f
